...
/*
  Número de threads executadas: 2
  Tempo sequencial: 5.015 segundos
  Tempo paralelo: 2.009
  Speedup obtido: 2,496
*/
/* 
...
// A utility function to find maximumimum of two integers
  ...
  for (int i=0; i<=m; i++) {
    for (int j=0; j<=n; j++) {
      if (i == 0 || j == 0)
        ...
        result = maximum(result, LCSuff[i][j]);
      }
      else LCSuff[i][j] = 0;
    }
  }
