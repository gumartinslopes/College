...
class lendoHtml{
  ...
  public static int contaTag(String tag, String s){
	...
	for(int i = 0; i < tag.length(); i++){
	 if(s.charAt(i) == tag.charAt(0) && s.charAt(i+1) == tag.charAt(1) && s.charAt(i +2) == tag.charAt(2) && s.charAt(i + 3) == tag.charAt(3)) //reduz comparacoes
  ...
  public static int contaNome(String nome, String s){
  	int total =0;
	for(int i = 0; i < s.length(); i++){
	   if(s.charAt(i) == nome.charAt(0) && s.charAt(i+1) == nome.charAt(1) && s.charAt(i +2) == nome.charAt(2) && s.charAt(i + 3) == nome.charAt(3)) //reduz comparacoes
              if(compara(i, 0 ,nome, s))
		   total ++;
	}
	return total;
  }
      ...
      do{
	   ...
	   endereco = MyIO.readString();
           ...
           escreve('i',contaVogal('i',html));   //3
      	   ...
      	   escreveString(nome, contaNome(nome,html) );//26
      	   MyIO.print("\n");
