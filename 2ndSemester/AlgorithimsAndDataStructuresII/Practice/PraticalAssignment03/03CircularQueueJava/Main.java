...
public class Main{
  ...
  public static Queue myTracks = new Queue();
     ...
     String id = new String();
      do{
        ...
        if(id.compareTo("FIM") != 0)
          myTracks.insert(getRegister(totalSearchById(id)));
  ...
  public static void operate(String input)throws Exception{
    ...
    if(operation.charAt(0) == 'I')
      myTracks.insert(getRegister(totalSearchById(commands[1])));
    
    else if(operation.charAt(0) == 'R')
      MyIO.println("(R) " + myTracks.remove().getName());
...
class Queue{
  ...
  public Queue(int maxLength){
    ...
    this.queue = new Music[maxLength];
  }

  public void display(){
    int i = first;
    while(i != last){
      MyIO.println("[" + i + "] " + queue[i].toString());
      i = ((i + 1) % queue.length);
    }
  }
  ...
  public Music remove(){
    Music removed = new Music();
    if(last == first)
      MyIO.println("A fila está vazia, não há itens para remover.");
    else{
      removed = queue[first];
