...
public class Main{
  ...
  public static void main(String[] args){
    ...
    try{
      ...
      createLog(executionTime,"690773_QuickSortParcial.txt");
    }
  ...
  public static boolean decide(Music a, Music b){
    boolean result;
    if(a.getDuration_ms() > b.getDuration_ms()){
      result = true;
      totalComparisons++;
    }
    else if(a.getDuration_ms() == b.getDuration_ms()){
  ...
  //metodo de ordenacao por insercao pelo atributo id 
  public static void sort(){
    partialQuickSort(0, playlistLength - 1);
  }
  
  public static void partialQuickSort(int left,int right){
    int i = left, j = right;
    Music pivot = playlist[(left + right)/2];
    
    //fase de particionamento
    while(i <= j){
      while(decide(pivot, playlist[i]))
        i++;
      while(decide(playlist[j],pivot))
        j--;
      if(i <= j){ 
        swap(i, j); 
        i++; 
    ...
    //fase recursiva
    if(left < j)
      partialQuickSort(left, j);
    if(i < k && i < right)
      partialQuickSort(i, right);
   }
