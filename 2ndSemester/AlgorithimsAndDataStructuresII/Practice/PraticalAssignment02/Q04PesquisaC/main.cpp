...
void inputSearch(Music playlist[], int playlist_length, int* comp);
bool binarySearch(Music playlist[], int playlist_length, char* key, int* comp);
void displayPlaylist(Music playlist[], int n);
void createlog(char *filepath, int comp, double time);
bool isSorted(Music playlist[], int n);
void swap(Music playlist[], int a, int b);
void sort(Music playlist[], int n);
...
int main(){
  ...
  //alocacao da lista completa de músicas 
  char **totalMusicList; 
  ...
  begin = clock(); 
  sort(playlist,playlist_length);//ordenacao pre pesquisa
  inputSearch(playlist,playlist_length, &comp);
  end = clock();
  
  createlog("690773_pesquisaBinaria.txt", comp,((end - begin) / (double)CLOCKS_PER_SEC));
  free(totalMusicList);
  return 0;
}
...
void inputSearch(Music playlist[], int playlist_length, int* comp){
   ...
   do{
    scanf(" %[^\n]s",id);
    if(strcmp(id, "FIM") != 0)
      displayAnswer(binarySearch(playlist, playlist_length, id, comp));
   }while(strcmp(id, "FIM") != 0);
}

//funcao de pesquisa binaria que responde a questao 04
bool binarySearch(Music playlist[], int playlist_length, char* key, int* comp){
  bool result = false;
  int left = 0, right = (playlist_length - 1), mid;
  while(left <= right){
    mid = (left + right)/2;
    *comp += 1;
    if(strcmp(key, playlist[mid].id) == 0){
      left = playlist_length;
      result = true;
    }
    else if(strcmp(key, playlist[mid].id) > 0){
...
void createlog(char *filepath,int comp, double time){
  ...
  fprintf(fileptr, "690773\t %d \t %f",comp,time); 
  fclose(fileptr);
}
...
bool isSorted(Music playlist[], int n){
  bool result = true;
  for(int i = 0; i < n - 1; i++){
    if(strcmp(playlist[i].id, playlist[i + 1].id) > 0){
      result = false;
      i = n - 1;
    }
  }
...
void quicksort(Music playlist[], int left, int right){
  int i = left, j = right;
  Music pivot = playlist[(left + right)/2];
  //particionamento
  while(i <= j){
    while(strcmp(playlist[i].id, pivot.id) < 0){
      i++;
    }
    while(strcmp(playlist[j].id, pivot.id) > 0){
      j--;
    }
    if(i <= j){
      swap(playlist,i, j);
  ...
  //recursao
  if(left < j)
    quicksort(playlist, left, j);
  if(i < right)
    quicksort(playlist, i, right);
}

//funcao que utiliza a ordemacao por quicksort 
void sort(Music playlist[], int n){
...
void readAtribute(int *stringIndex, char *infos, char* output){
  ...
  while(infos[i] != '\0' && ((infos[i] != ',') || !(infos[i] == ',' && infos[i + 1] != ' '))){ //pulamos virgulas e quotes desnecessarios
    if(!(infos[i] == '"' && (infos[i + 1] == '[' || infos[i - 1] == ']'))) 
      output[j++] = infos[i];  
    i++;
  }
   output[j] = '\0';
