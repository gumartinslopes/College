...
public class Main{
  ...
  public static int playlistLength = 1;
  ...
  public static void main(String[] args){
    ...
    try{
      ...
      createLog(executionTime,"690773_heapsort.txt");
    }
  ...
  public static boolean isSorted(){
    boolean result = true;
    for(int i = 1; i < playlistLength - 1; i++){
  ...
  public static void sort(){
    for(int size = 2; size <= playlistLength - 1; size++){
      build(size);
    }
    int size = playlistLength - 1;
    while(size > 1){
      swap(1, size--);
      rebuild(size);
    }
  ...
  public static void rebuild(int size){
    int i = 1;
    while(i <= (size/2)){
  ...
  //funcao que decide se playlist[j] > aux
  public static boolean decide(Music a, Music b){
    boolean result;
    totalComparisons++;       //apenas a comparacao do id esta sendo contabilizada
    if(a.getRelease_date().compareDate(b.getRelease_date()) > 0)
      result = true;
    else if(a.getRelease_date().compareDate(b.getRelease_date()) == 0)
  ...
  public static int getBiggestChild(int i, int size){
    int child;
    totalComparisons++;
    if(2 * i == size || (playlist[2 * i].getRelease_date().compareDate(playlist[2 * i + 1].getRelease_date()) > 0))
      child = 2 * i;
    else
      child = 2 * i + 1;
    return child;
...
class SimpleDate{
  ...
  public int compareDate(SimpleDate d){
    int result = 0;
