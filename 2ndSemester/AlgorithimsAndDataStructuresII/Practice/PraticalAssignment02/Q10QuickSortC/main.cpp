...
int main(){
  ...
  createlog("690773_quicksort.txt", comp, mov, execution_time);
...
void quicksort(Music playlist[], int left, int right, int *mov, int *comp){
  int i = left, j = right;
  Music pivot = playlist[(left + right)/2];
  //particionamento
  while(i <= j){
    while(playlist[i].duration_ms < pivot.duration_ms){
      *comp+= 1;
      i++;
    }
    while(playlist[j].duration_ms > pivot.duration_ms){
      *comp+= 1;
      j--;
    }
    if(i <= j){
  ...
  //recursao
  if(left < j)
    quicksort(playlist, left, j, comp, mov);
  if(i < right)
    quicksort(playlist, i, right, comp, mov);
}

//funcao que utiliza a ordemacao por quicksort 
void sort(Music playlist[], int n, int* comp, int* mov){
  quicksort(playlist, 0, n - 1, comp, mov);
}
