...
public class Main{
  ...
  public static void main(String[] args){
    ...
    try{
      ...
      createLog(executionTime,"690773_shellsort.txt");
    }
  ...
  public static void sort(){
    ...
    }while(h != 1); //array ja ordenado
  }
 
  //metodo de insercao por cor 
  public static void insertionByColor(int color, int h){
    for(int i = (h + color); i < playlistLength; i += h){
      Music aux = playlist[i].clone();
      int j = i - h;
      while((j >= 0) && decide(playlist[j], aux)){
        playlist[j + h] = playlist[j].clone();
        totalMoves++;
        j-= h;
      }
      playlist[j + h] = aux.clone();
      totalMoves++;
    }
  }
