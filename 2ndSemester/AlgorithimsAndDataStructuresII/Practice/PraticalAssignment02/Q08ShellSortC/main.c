...
int main(){
  ...
  createlog("690773_shellsort.txt", comp, mov, execution_time);
...
void insertionByColor(Music playlist[],int n, int color, int h,int* comp, int* mov){
  for(int i = (h + color); i < n; i += h){
    Music aux = playlist[i];
    int j = i - h;
    while((j >= 0) && decide(&playlist[j], &aux)){
      playlist[j + h] = playlist[j];
      *mov += 1;
      j-= h;
    }
    playlist[j + h] = aux;
...
//funcao que utiliza a ordemacao por bolha otimizada 
void sort(Music playlist[], int n, int* comp, int* mov){
  ...
  do{
    ...
    for(int color = 0; color < h; color++){
      insertionByColor(playlist, n, color, h, comp, mov);
    }
  }while(h != 1);
}
