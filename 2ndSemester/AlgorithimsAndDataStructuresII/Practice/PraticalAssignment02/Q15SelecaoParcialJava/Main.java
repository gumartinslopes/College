...
public class Main{
  ...
  public static void main(String[] args){
    ...
    try{
      ...
      createLog(executionTime,"690773_selecaoParcial.txt");
    }
  ...
  public static void displayPlaylist(int k){
    for(int i = 0; i < k; i++){
      playlist[i].imprimir();
    }
  }
  ...
  //metodo de ordenacao por selecao parcial
  public static void sort(int k){
    for(int i = 0; i < k; i++){
      int minor = i;
      totalComparisons++;
      for(int j = (i + 1); j < playlistLength; j++){
        if(playlist[minor].getName().compareTo(playlist[j].getName()) > 0){
          minor = j;
        }
      }
      swap(minor, i);
