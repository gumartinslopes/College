...
int main(){
  ...
  createlog("690773_selecaoRecursiva.txt", comp, mov, execution_time);
...
int recursiveSelectionSort(Music playlist[], int n, int i, int *comp, int* mov){
  if(i < n - 1){//condicao de parada da recursao
    int smaller = i;
    for(int j = (i + 1); j < n; j++){
      *comp += 1;
      if(strcmp(playlist[smaller].name, playlist[j].name) > 0){
        smaller = j;
      }
    }
    ...
    *comp += 1;
    ...
    swap(playlist, smaller, i);
    *mov += 3;
    recursiveSelectionSort(playlist, n, i + 1, comp, mov);
  }
}


//funcao que utiliza a ordemacao por insercao recursiva
void sort(Music playlist[], int n, int* comp, int* mov){
  recursiveSelectionSort(playlist, n, 0, comp, mov);
}
